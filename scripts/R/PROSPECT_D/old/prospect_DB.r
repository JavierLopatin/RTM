...
prospect_DB = function(N,Cab,Car,Ant,Brown,Cw,Cm){
  ...
  # 11:545-556.
  # ***********************************************************************
  D       = sqrt((1+ra+ta)*(1+ra-ta)*(1-ra+ta)*(1-ra-ta))
  rq      = ra^2
  tq      = ta^2
  a       = (1+rq-tq+D)/(2*ra)
  b       = (1-rq+tq+D)/(2*ta)
  ...
  j       = which(ra+ta >= 1)
  Tsub[j] = ta[j]/(ta[j]+(1-ta[j])*(N-1))
  Rsub[j]	= 1-Tsub[j]
  
  # Reflectance and transmittance of the leaf: combine top layer with next N-1 layers
  denom   = 1-Rsub*ra
  tran    = Ta*Tsub/denom
  refl    = Ra+Ta*Rsub*ta/denom
  
  # Return wavelength, reflectance and transmittance
  #LRT = list()
  #LRT[[1]] = lambda
  #LRT[[2]] = refl
  #LRT[[3]] = tran
